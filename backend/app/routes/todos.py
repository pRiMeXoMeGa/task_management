...
def get_todos_by_todo_status(
            status:str, 
            db:Session=Depends(database.get_db),
            logged_user:Session = Depends(oauth2.get_current_user)
            ):
    todos = db.query(models.Todo).filter(logged_user.id==models.Todo.user_id,
                                         status==models.Todo.status).all()
    if todos== None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"no todo list found")
    return {
...
def update_todos(
    ...
    if todo.status not in ['To Do', 'In Progress', 'Done']:
        raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE,
                            detail=f"Status of the task must be either 'To Do', 'In Progress', or 'Done' only")
